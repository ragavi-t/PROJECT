...
if(isset($btn))
{
 if(trim($name)=="") { $msg = "Enter the Name..."; }
	...
	$rdate=date("Y-m-d");
		...
		$uqry="insert into register(id,utype,name,gender,address,pincode,contact,email,bank_name,acc_no,username,password,rdate) values($id,'user','$name','$gender','$address','$pincode',$contact,'$email','$bank_name','$acc_no','$uname','$pwd','$rdate')";
...
function validate()
{
	...
	///////////////////
	/*var inputtxt=document.form1.pwd;
	...
	{ 
	alert('Password must alphabet,number,special character');
	document.form1.pwd.select();
	return false;
	}*/
	
	////////////
	if(document.form1.cpass.value!=document.form1.pwd.value)
	{
